package com.glow.gbguimanager.render;
    ...
    public TextBuilder draw(GuiGraphics graphics) {
        ...
        if (fontRenderer == null) {
            ...
            if (shadow && color.getAlpha() - damp > 0) {
                ...
                Color shadowColor = new Color(Math.clamp(color.getRed() - damp,0,255), Math.clamp(color.getGreen() - damp,0,255), Math.clamp(color.getBlue() - damp,0,255), Math.clamp(color.getAlpha() - 50,0,255));
                if (dynamic) fontRenderer.drawDynamicString(graphics,text,pos.getAdded(1,1).getMultiplied(1/scale),shadowColor);
                else fontRenderer.drawStaticString(graphics,text,pos.getAdded(1,1).getMultiplied(1/scale),shadowColor);
            }
