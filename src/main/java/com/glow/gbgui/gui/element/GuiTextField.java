...
import com.glow.gbgui.baselib.action.StopWatch;
...
public class GuiTextField extends GuiWidget {
    ...
    private final Container<String> valueContainer;
    ...
    private GuiTextField(Screen screen, String title, Vector pos, Vector size, Container<String> valueContainer, boolean numbersOnly) {
    ...
    protected void drawWidget(GuiGraphics graphics, Vector mousePos) {
        ...
        DrawUtil.drawRectangle(graphics,
                getPos().getAdded(new Vector(2 + DrawUtil.getFontTextWidth(getTitle() + ": ",fontRenderer), getSize().getY()- 2)),
                getSize().getAdded(new Vector(- 4 - DrawUtil.getFontTextWidth(getTitle() + ": ",fontRenderer),1-getSize().getY())),
                Color.WHITE);

        String title = getTitle() + ": ";
        String msg = getContainer().get();
        ...
        if (typing) {
            ...
            double x = (DrawUtil.getFontTextWidth(title.concat(msg),fontRenderer) > getSize().getX() - 4)
                    ? getPos().getX() + getSize().getX() - 1
                    : (getPos().getX() + 3 + DrawUtil.getFontTextWidth(getTitle() + ": ",fontRenderer) + DrawUtil.getFontTextWidth(getContainer().get(),fontRenderer));
            DrawUtil.drawRectangle(graphics, new Vector(x,getPos().getY() + 3),new Vector(1,getSize().getY() - 6),new Color(255,255,255,alpha));
        }

        //Draw Text
        double scale = 1;
        if (DrawUtil.getFontTextWidth(title.concat(msg),fontRenderer) > getSize().getX() - 2) scale = (getSize().getX() - 2 - DrawUtil.getFontTextWidth(title,fontRenderer))/(DrawUtil.getFontTextWidth(msg,fontRenderer));

        TextBuilder
                .start(title,getPos().getAdded(new Vector(2, 1 + getSize().getY() / 2 - DrawUtil.getFontTextHeight(fontRenderer) / 2)),Color.WHITE).draw(graphics)
    ...
    public void keyPressed(int key, int scancode, int modifiers) {
        ...
        switch (key) {
            ...
            case GLFW.GLFW_KEY_SPACE -> {
                if (isValidPress(key,numbersOnly)) buttonText = buttonText + " ";
            }

            default -> {
                if (GLFW.glfwGetKeyName(key, -1) == null || GLFW.glfwGetKeyName(key, -1).equals("null")) break;
                if (isValidPress(key,numbersOnly)) buttonText = buttonText + GLFW.glfwGetKeyName(key, -1);
            }
        }
        getContainer().set(buttonText);
    }

    public static boolean isValidPress(int key, boolean isNumbersOnly) {
        if (isNumbersOnly) {
